...
def detect_swaps(diffs: list[FlowDiff]) -> list[FlowDiff]:
    ...
    for i in range(len(old_indices) - 1):
        ...
        if diff_a.new_index == diff_b.new_index + 1 and diff_b.new_index == diff_a.new_index - 1:
...
def compute_order_changes(old_tests: list[TestItem], new_tests: list[TestItem]) -> list[str]:
    ...
    common = sorted(
        set(old_first_pos.keys()) & set(new_first_pos.keys()), key=lambda n: old_first_pos[n]
    )
